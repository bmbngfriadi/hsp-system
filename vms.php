...
<head>
  ...
  <style>
    ...
    .stats-card:hover { transform: translateY(-4px); box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05); }
...
<body class="bg-slate-50 text-slate-800 h-screen flex flex-col overflow-hidden">
           ...
           <div class="hidden md:block overflow-x-auto">
             <table class="w-full text-left text-sm whitespace-nowrap">
               <thead class="bg-slate-50 border-b border-slate-200 text-slate-500 uppercase text-xs font-bold">
                 <tr><th class="px-6 py-4" data-i18n="th_id">ID & Date</th><th class="px-6 py-4" data-i18n="th_user">User Info</th><th class="px-6 py-4" data-i18n="th_unit">Unit & Purpose</th><th class="px-6 py-4" data-i18n="th_approval">Approval Status</th><th class="px-6 py-4 text-center" data-i18n="th_status">Status</th><th class="px-6 py-4 text-center" data-i18n="th_trip">Trip Info</th><th class="px-6 py-4 text-right min-w-[160px]" data-i18n="th_action">Action</th></tr>
               </thead>
  ...
  <script>
    ...
    document.addEventListener('keydown', function(event) {
        if (event.key === "Escape") {
            const modals = ['modal-create', 'modal-export', 'modal-trip', 'modal-confirm', 'modal-alert', 'modal-cancel'];
    ...
    function loadData() { 
        document.getElementById('data-table-body').innerHTML = '<tr><td colspan="7" class="text-center py-10 text-slate-400"><span class="loader-spin mr-2"></span> Fetching data...</td></tr>'; 
        fetch('api/vms.php', {
            method: 'POST',
            body: JSON.stringify({ 
                action: 'getData', 
        ...
        .then(res => {
            if(res.success) {
                ...
                document.getElementById('data-table-body').innerHTML = `<tr><td colspan="7" class="text-center py-10 text-red-500">Error: ${res.message}</td></tr>`; 
            }
        }).catch(err => {
             console.error(err);
             document.getElementById('data-table-body').innerHTML = `<tr><td colspan="7" class="text-center py-10 text-red-500">Connection Error</td></tr>`;
        });
    }
    ...
    function renderFleetStatus(vehicles) { const container = document.getElementById('fleet-status-container'); container.innerHTML = ''; if(vehicles.length === 0) { container.innerHTML = '<div class="text-slate-500 text-sm italic">No fleet available.</div>'; return; } vehicles.forEach(v => { let colorClass = 'bg-white border-slate-200 text-slate-600', icon = 'fa-car', statusText = 'Unknown'; if(v.status === 'Available') { colorClass = 'bg-green-50 border-green-200 text-green-700'; icon = 'fa-check-circle'; statusText = 'Available'; } else if (v.status === 'In Use') { colorClass = 'bg-blue-50 border-blue-200 text-blue-700'; icon = 'fa-road'; statusText = 'In Use'; } else if (v.status === 'Reserved') { colorClass = 'bg-yellow-50 border-yellow-200 text-yellow-700'; icon = 'fa-clock'; statusText = 'Reserved'; } else { colorClass = 'bg-red-50 border-red-200 text-red-700'; icon = 'fa-ban'; statusText = 'Maintenance'; } container.innerHTML += `<div class="${colorClass} border p-4 rounded-xl shadow-sm h-full flex flex-col justify-between transition-all duration-300 hover:-translate-y-1 hover:shadow-md cursor-default"><div><div class="flex justify-between items-start mb-2"><div><div class="font-bold text-sm text-slate-800">${v.plant}</div><div class="text-[10px] uppercase font-semibold opacity-70 mt-0.5">${v.model}</div></div><i class="fas ${icon} text-lg opacity-50"></i></div><div class="text-right text-xs font-bold mb-1">${statusText}</div></div></div>`; }); }
    
    function renderTable(data) {
       const tbody = document.getElementById('data-table-body'); const cardCont = document.getElementById('data-card-container'); tbody.innerHTML = ''; cardCont.innerHTML = ''; if (data.length === 0) { tbody.innerHTML = '<tr><td colspan="7" class="text-center py-10 text-slate-400 italic">No data found.</td></tr>'; cardCont.innerHTML = '<div class="text-center py-10 text-slate-400 italic">No data found.</div>'; return; }
       const getStatusBox = (role, text, time) => { let cls = "bg-gray-50 text-gray-400 border-gray-200", icon = "fa-minus"; let txt = text || "Pending"; if(txt.includes('Approved')) { cls="bg-green-50 text-green-700 border-green-200"; icon="fa-check"; } else if(txt.includes('Pending')) { cls="bg-yellow-50 text-yellow-600 border-yellow-200"; icon="fa-clock"; } else if(txt.includes('Rejected')) { cls="bg-red-50 text-red-700 border-red-200"; icon="fa-times"; } let displayTxt = txt.replace('Approved by ', '').replace('Rejected by ', ''); if(displayTxt === 'Pending') displayTxt = 'Pending'; const timeHtml = time ? `<div class="text-[9px] text-slate-400 mt-1 font-mono tracking-tighter">${time}</div>` : ''; return `<div class="flex flex-col"><div class="app-box ${cls}"><i class="fas ${icon} text-xs w-3"></i><span class="text-[10px] font-bold uppercase leading-none" title="${txt}">${displayTxt.substring(0,8)}</span></div>${timeHtml}</div>`; };
       ...
       data.forEach(row => {
          ...
          if (currentUser.role === 'HRGA') { 
              if (status === 'Pending GA') {
                  const b = renderApprovalBtns('HRGA');
                  actionBtn = b.pc; actionBtnMobile = b.mob;
              }
          ...
          if (status === 'Pending Section Head' && currentUser.department === 'HRGA') {
              if (currentUser.role === 'TeamLeader' || currentUser.role === 'HRGA') {
                   const b = renderApprovalBtns('HRGA Approver');
                   actionBtn = b.pc; actionBtnMobile = b.mob;
              }
          }
          ...
          const gaBox = getStatusBox('GA', row.appGa, row.gaTime); const headBox = getStatusBox('S.HEAD', row.appHead, row.headTime); 
          const commentHtml = row.actionComment ? `<div class="mt-1 text-[10px] text-slate-500 italic"><i class="fas fa-comment-dots text-slate-400 mr-1"></i> "${row.actionComment}"</div>` : ''; 
          ...
          tbody.innerHTML += `<tr class="hover:bg-slate-50 transition border-b border-slate-50 align-top"><td class="px-6 py-4"><div class="font-bold text-xs text-slate-700">${timestamp}</div><div class="text-[10px] text-slate-400">#${idStr}</div></td><td class="px-6 py-4"><div class="font-bold text-xs text-slate-700">${row.username}</div><div class="text-[10px] text-slate-500">${row.department}</div></td><td class="px-6 py-4"><div class="text-xs font-bold text-blue-700 bg-blue-50 px-1 rounded inline-block mb-1">${row.vehicle}</div><div class="text-xs text-slate-600 italic truncate max-w-[150px]" title="${row.purpose}">${row.purpose}</div></td><td class="px-6 py-4"><div class="flex flex-col gap-3 w-28"><div class="flex items-start gap-2"><span class="text-[9px] w-6 font-bold text-slate-400 mt-1">GA</span>${gaBox}</div><div class="flex items-start gap-2"><span class="text-[9px] w-6 font-bold text-slate-400 mt-1">HEAD</span>${headBox}</div>${commentHtml}</div></td><td class="px-6 py-4 text-center"><span class="status-badge ${badge}">${status}</span></td><td class="px-6 py-4 text-center">${photosHtml}</td><td class="px-6 py-4 text-right align-top min-w-[160px]">${actionBtn}</td></tr>`;
          ...
          cardCont.innerHTML += `
            <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-200 relative">
               ...
               <div class="grid grid-cols-2 gap-3 mb-4">
                  <div>
                     <div class="text-[9px] font-bold text-slate-400 mb-1">GA APPROVAL</div>
                     ${gaBox}
                  </div>
                  <div>
               ...
               ${row.actionComment ? `<div class="mb-3 text-xs text-slate-500 italic bg-slate-50 p-2 rounded border border-dashed border-slate-200"><i class="fas fa-comment text-slate-400 mr-1"></i> ${row.actionComment}</div>` : ''}
    ...
    function submitData() { 
        const v = document.getElementById('input-vehicle').value; 
        const p = document.getElementById('input-purpose').value; 
        ...
        .then(res => {
            ...
            if(res.success) { 
                closeModal('modal-create'); loadData(); showAlert("Success", "Request sent."); 
            } else { 
    ...
    function callUpdate(id, act, comment) {
        fetch('api/vms.php', {
            method: 'POST',
            body: JSON.stringify({ action: 'updateStatus', id: id, act: act, userRole: currentUser.role, extraData: {comment: comment} })
        }).then(() => loadData());
    }

    function approve(id, role) { showConfirm("Confirm Action", "Are you sure?", (comment) => { callUpdate(id, 'approve', comment); }); }
    function reject(id, role) { showConfirm("Confirm Action", "Are you sure?", (comment) => { callUpdate(id, 'reject', comment); }); }
    ...
    function sendTripData(id, act, km, photoBase64, route) { 
        const btn = document.getElementById('btn-trip-submit'); btn.disabled = true; 
        fetch('api/vms.php', {
            method: 'POST',
            body: JSON.stringify({
                action: 'updateStatus',
        ...
        .then(r => r.json())
        .then(res => {
            btn.disabled = false; closeModal('modal-trip'); loadData(); 
        });
    }
