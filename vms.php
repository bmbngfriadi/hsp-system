  ...
  <script>
    ...
    function exportExcel(data) {
        ...
        rows.push([
            ...
            "Current Status", 
            "L1 (Dept) Approver", "L1 Time", 
            "L2 (HRGA) Approver", "L2 Time",
            "L3 (Final) Approver", "L3 Time",
    ...
    function exportPdf(data) {
        ...
        const body = data.map(r => [
            ...
            r.status,
            r.l1By || '-', r.l2By || '-', r.l3By || '-'
        ]);
        
        doc.autoTable({
            startY: 25,
            head: [['ID', 'Date', 'User', 'Vehicle', 'Purpose', 'KM (Start-End)', 'Status', 'L1 (Dept)', 'L2 (HRGA)', 'L3 (Final)']],
            body: body,
            theme: 'grid',
    ...
    function renderTable(d){
        ...
        // Block Status Logic
        const gSB=(label,status,tm,bn)=>{
            let cl="bg-gray-50 text-gray-400 border-gray-200",ic="fa-minus",tx=status||"Pending";
            if(tx.includes('Approved')||tx==='Auto-Skip'){cl="bg-green-50 text-green-700 border-green-200";ic="fa-check";}
            else if(tx.includes('Pending')){cl="bg-yellow-50 text-yellow-600 border-yellow-200";ic="fa-clock";}
            else if(tx.includes('Rejected')){cl="bg-red-50 text-red-700 border-red-200";ic="fa-times";}
            
            let dt = tx;
            if(dt.length > 8 && dt !== 'Auto-Skip') dt = dt.replace('Approved','OK').replace('Rejected','NO').substring(0,8);
            if(dt==='Pending') dt='Pending';
            
            let ah='';
            if((tx==='Approved'||tx.includes('Rejected')) && bn) ah=`<div class="text-[9px] text-slate-500 mt-0.5 truncate w-20" title="${bn}">${bn}</div>`;
            return `<div class="flex flex-col"><span class="text-[9px] font-bold text-slate-400 mb-0.5">${label}</span><div class="app-box ${cl}"><i class="fas ${ic} text-xs w-3"></i><span class="text-[10px] font-bold uppercase leading-none" title="${tx}">${dt}</span></div>${ah}</div>`;
        };
        ...
        d.forEach(r=>{
            ...
            if(s==='Done'||s==='Approved')b='bg-emerald-50 text-emerald-700 border-emerald-200';
            else if(s==='Active')b='bg-blue-50 text-blue-700 border-blue-200';
            else if(s==='Rejected'||s==='Cancelled')b='bg-red-50 text-red-700 border-red-200';
            else if(s.includes('Pending')||s==='Correction Needed'||s==='Pending Review')b='bg-amber-50 text-amber-700 border-amber-200';
            
            let ab='',abm='';
            const rAB=(t)=>{
                return {
                    pc: `<div class="flex items-center gap-2 w-full mt-1"><button onclick="approve('${r.id}','${t}')" class="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white px-2 py-1.5 rounded-lg text-xs font-bold shadow-sm btn-action flex items-center justify-center gap-1 transition"><i class="fas fa-check"></i> OK</button><button onclick="reject('${r.id}','${t}')" class="flex-1 bg-red-600 hover:bg-red-700 text-white px-2 py-1.5 rounded-lg text-xs font-bold shadow-sm btn-action flex items-center justify-center gap-1 transition"><i class="fas fa-times"></i> NO</button></div>`,
                    mob: `<div class="flex flex-col gap-2 mt-2"><button onclick="approve('${r.id}','${t}')" class="w-full bg-emerald-600 text-white py-3 rounded-lg text-sm font-bold shadow-sm"><i class="fas fa-check"></i> Approve</button><button onclick="reject('${r.id}','${t}')" class="w-full bg-red-600 text-white py-3 rounded-lg text-sm font-bold shadow-sm"><i class="fas fa-times"></i> Reject</button></div>`
                };
            };

            // --- APPROVAL BUTTON LOGIC (3 LEVEL) ---
            // L1: Pending Dept Head -> User's Dept SectionHead OR TeamLeader
            if (s === 'Pending Dept Head') {
                if (r.department === currentUser.department && (currentUser.role === 'SectionHead' || currentUser.role === 'TeamLeader')) {
                    const x = rAB('L1'); ab=x.pc; abm=x.mob;
                }
            }
            // L2: Pending HRGA -> Role HRGA
            else if (s === 'Pending HRGA') {
                if (currentUser.role === 'HRGA' && currentUser.department === 'HRGA') {
                     const x = rAB('L2'); ab=x.pc; abm=x.mob;
                }
            }
            // L3: Pending Final -> TeamLeader HRGA (or Backup HRGA)
            else if (s === 'Pending Final') {
                if (currentUser.department === 'HRGA' && (currentUser.role === 'TeamLeader' || currentUser.role === 'HRGA')) {
                     const x = rAB('L3'); ab=x.pc; abm=x.mob;
                }
            }
            // HRGA Review Trip
            else if (currentUser.role === 'HRGA' && s === 'Pending Review') {
                ab=`<div class="flex items-center gap-2 w-full mt-1"><button onclick="confirmTrip('${r.id}')" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white px-2 py-1.5 rounded-lg text-xs font-bold shadow-sm btn-action"><i class="fas fa-check-double mr-1"></i> Verify</button><button onclick="requestCorrection('${r.id}')" class="flex-1 bg-orange-500 hover:bg-orange-600 text-white px-2 py-1.5 rounded-lg text-xs font-bold shadow-sm btn-action"><i class="fas fa-edit mr-1"></i> Rev</button></div>`;
                abm=`<div class="flex flex-col gap-2 mt-2"><button onclick="confirmTrip('${r.id}')" class="w-full bg-blue-600 text-white py-3 rounded-lg text-sm font-bold shadow-sm">Verify Done</button><button onclick="requestCorrection('${r.id}')" class="w-full bg-orange-500 text-white py-3 rounded-lg text-sm font-bold shadow-sm">Request Correction</button></div>`;
            }

            // User Self Action
            if(r.username===currentUser.username){
                if(s==='Approved'){
                    ab=`<div class="flex gap-2 justify-end items-center mt-1"><button onclick="openTripModal('${r.id}', 'startTrip', '${r.startKm}')" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-lg text-xs font-bold shadow-sm btn-action flex items-center justify-center gap-1"><i class="fas fa-play text-[10px]"></i> Start</button><button onclick="openCancelModal('${r.id}')" class="bg-white border border-slate-300 text-slate-500 hover:text-red-600 hover:border-red-300 px-2 py-1.5 rounded-lg text-xs font-bold btn-action transition"><i class="fas fa-times"></i></button></div>`;
                    abm=`<div class="flex gap-2 mt-2"><button onclick="openTripModal('${r.id}', 'startTrip', '${r.startKm}')" class="flex-1 bg-blue-600 text-white py-3 rounded-lg text-sm font-bold shadow-sm flex items-center justify-center gap-2"><i class="fas fa-play"></i> Start Trip</button><button onclick="openCancelModal('${r.id}')" class="bg-slate-200 text-slate-600 px-4 py-3 rounded-lg text-sm font-bold shadow-sm"><i class="fas fa-times"></i></button></div>`;
                } else if(s==='Active'){
                    ab=`<button onclick="openTripModal('${r.id}', 'endTrip', '${r.startKm}')" class="w-full bg-orange-600 hover:bg-orange-700 text-white px-3 py-1.5 rounded-lg text-xs font-bold shadow-sm btn-action flex items-center justify-center gap-1 mt-1"><i class="fas fa-flag-checkered text-[10px]"></i> Finish Trip</button>`;
                    abm=`<button onclick="openTripModal('${r.id}', 'endTrip', '${r.startKm}')" class="w-full bg-orange-600 text-white py-3 rounded-lg text-sm font-bold shadow-sm flex items-center justify-center gap-2 mt-2"><i class="fas fa-flag-checkered"></i> Finish Trip</button>`;
                } else if(s==='Correction Needed'){
                    ab=`<button onclick="openTripModal('${r.id}', 'submitCorrection', '${r.startKm}')" class="w-full bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold shadow-sm btn-action flex items-center justify-center gap-1 mt-1"><i class="fas fa-tools text-[10px]"></i> Fix Data</button>`;
                    abm=`<button onclick="openTripModal('${r.id}', 'submitCorrection', '${r.startKm}')" class="w-full bg-yellow-500 text-white py-3 rounded-lg text-sm font-bold shadow-sm flex items-center justify-center gap-2 mt-2"><i class="fas fa-tools"></i> Fix Data</button>`;
                } else if(s.includes('Pending')&&s!=='Pending Review'){
                    ab=`<button onclick="openCancelModal('${r.id}')" class="w-full bg-slate-400 hover:bg-slate-500 text-white px-3 py-1.5 rounded-lg text-xs font-bold shadow-sm btn-action flex items-center justify-center gap-2 mt-1"><i class="fas fa-ban"></i> Cancel Request</button>`;
                    abm=`<button onclick="openCancelModal('${r.id}')" class="w-full bg-slate-400 text-white py-3 rounded-lg text-sm font-bold shadow-sm flex items-center justify-center gap-2 mt-2"><i class="fas fa-ban"></i> Cancel Request</button>`;
                }
            }
            ...
            const bL1=gSB('L1 (Dept)',r.appL1,r.l1Time,r.l1By);
            const bL2=gSB('L2 (HRGA)',r.appL2,r.l2Time,r.l2By);
            const bL3=gSB('L3 (Final)',r.appL3,r.l3Time,r.l3By);
    ...
    // ... (Sisa fungsi JS lainnya sama seperti sebelumnya) ...
